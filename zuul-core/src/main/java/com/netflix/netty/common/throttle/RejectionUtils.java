 ...
 * coupled with the channel pipeline, but can be called from different handlers.
 */
public final class RejectionUtils {

  // TODO(carl-mastrangelo): add tests for this.

  public static final HttpResponseStatus REJECT_CLOSING_STATUS =
      new HttpResponseStatus(999, "Closing(Rejection)");

  /**
   * Closes the connection without sending a response, and fires a {@link RequestRejectedEvent} back
   * up the pipeline.
   *
   ...
   * @param injectedLatencyMillis optional parameter to delay sending a response. The reject
   *     notification is still sent up the pipeline.
   */
  ...
  /**
   * Sends a rejection response back to the client, and fires a {@link RequestRejectedEvent} back up
   * the pipeline.
   *
  ...
  public static void sendRejectionResponse(
    ...
    FullHttpResponse response =
        createRejectionResponse(rejectedCode, rejectedBody, shouldClose, rejectionHeaders);
  ...
  /**
   * Throttle either by sending rejection response message, or by closing the connection now, or
   * just drop the message. Only call this if ThrottleResult.shouldThrottle() returned {@code true}.
   *
   * @param ctx the channel handler processing the request
  ...
  private static void notifyHandlers(
    ...
    // Send this from the beginning of the pipeline, as it may be sent from the
  ...
  private static FullHttpResponse createRejectionResponse(
    ...
    return new DefaultFullHttpResponse(
        HttpVersion.HTTP_1_1, status, body, headers, EmptyHttpHeaders.INSTANCE);
  }
