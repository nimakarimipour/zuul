...
/**
 * This handler times from the point a HttpRequest is read until the LastHttpContent is read, and
 * fires a HttpRequestTimeoutEvent if that time has exceed the configured timeout.
 *
 * <p>Unlike ReadTimeoutHandler, this impl does NOT close the channel on a timeout. Only fires the
 * event.
 *
 * @author michaels
 */
public class HttpRequestReadTimeoutHandler extends ChannelInboundHandlerAdapter {
  ...
  public static void addLast(
      ...
      long timeout,
      TimeUnit unit,
      Counter httpRequestReadTimeoutCounter) {
