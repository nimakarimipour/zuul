 ...
 * host + port).
 *
 * <p>User: michaels@netflix.com Date: 4/14/16 Time: 4:29 PM
 */
...
public final class SourceAddressChannelHandler extends ChannelInboundHandlerAdapter {
  ...
  public static final AttributeKey<SocketAddress> ATTR_REMOTE_ADDR =
      AttributeKey.newInstance("_remote_addr");
  ...
  /**
   * The host address of the source. This is derived from {@link #ATTR_REMOTE_ADDR}. If the address
   * is an IPv6 address, the scope identifier is absent.
   */
  public static final AttributeKey<String> ATTR_SOURCE_ADDRESS =
      AttributeKey.newInstance("_source_address");

  /**
   * Indicates the local address of the channel. This can be different than the one {@link Channel}
  ...
  public static final AttributeKey<SocketAddress> ATTR_LOCAL_ADDR =
      AttributeKey.newInstance("_local_addr");
  ...
  /**
   * The local address of this channel. This is derived from {@code channel.localAddress()}, or from
   * the Proxy Protocol preface if provided. If the address is an IPv6 address, the scope identifier
   * is absent. Unlike {@link #ATTR_SERVER_LOCAL_ADDRESS}, this value is overwritten with the Proxy
   * Protocol local address (e.g. the LB's local address), if enabled.
   */
  public static final AttributeKey<String> ATTR_LOCAL_ADDRESS =
      AttributeKey.newInstance("_local_address");

  /**
   * The actual local address of the channel, in string form. If the address is an IPv6 address, the
   * scope identifier is absent. Unlike {@link #ATTR_LOCAL_ADDRESS}, this is not overwritten by the
   * Proxy Protocol message if present.
   *
  ...
  /**
   * The port number of the local socket, or {@code -1} if not appropriate. This is not overwritten
   * by the Proxy Protocol message if present.
   *
   * @deprecated Use {@code channel.localAddress()} instead.
   */
  @Deprecated
  ...
  private InetSocketAddress sourceAddress(Channel channel) {
    ...
    if (null != remoteSocketAddr
        && InetSocketAddress.class.isAssignableFrom(remoteSocketAddr.getClass())) {
  ...
  private InetSocketAddress localAddress(Channel channel) {
    ...
    if (null != localSocketAddress
        && InetSocketAddress.class.isAssignableFrom(localSocketAddress.getClass())) {
