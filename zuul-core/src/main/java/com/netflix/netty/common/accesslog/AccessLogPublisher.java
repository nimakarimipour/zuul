...
public class AccessLogPublisher {
    ...
    private final static List<String> LOG_REQ_HEADERS = new DynamicStringListProperty("zuul.access.log.requestheaders", "host,x-forwarded-for,x-forwarded-proto,x-forwarded-host,x-forwarded-port,user-agent").get();

    private final static List<String> LOG_RESP_HEADERS = new DynamicStringListProperty("zuul.access.log.responseheaders", "server,via,content-type").get();

    private final static DynamicIntProperty URI_LENGTH_LIMIT = new DynamicIntProperty("zuul.access.log.uri.length.limit", Integer.MAX_VALUE);
    ...
    public void log(Channel channel, @Nullable() HttpRequest request, @Nullable() HttpResponse response, LocalDateTime dateTime, Integer localPort, String remoteIp, Long durationNs, Integer requestBodySize, Integer responseBodySize) {
