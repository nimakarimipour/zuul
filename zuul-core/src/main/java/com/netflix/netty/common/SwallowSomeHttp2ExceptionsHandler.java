...
public class SwallowSomeHttp2ExceptionsHandler extends ChannelOutboundHandlerAdapter {
  private static final Logger LOG =
      LoggerFactory.getLogger(SwallowSomeHttp2ExceptionsHandler.class);
  ...
  public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
    ...
    if (cause instanceof Http2Exception) {
      ...
      if (h2e.error() == Http2Error.NO_ERROR
        ...
        // This is the exception we threw ourselves to make the http2 codec gracefully close the
        // connection. So just
  ...
  private void incrementExceptionCounter(Throwable throwable) {
    registry
        .counter("server.connection.pipeline.exception", "id", throwable.getClass().getSimpleName())
        .increment();
  }
}
