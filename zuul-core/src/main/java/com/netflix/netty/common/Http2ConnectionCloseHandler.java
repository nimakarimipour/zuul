...
/** User: michaels@netflix.com Date: 2/8/17 Time: 2:03 PM */
...
public class Http2ConnectionCloseHandler extends ChannelDuplexHandler {
  ...
  /**
   * Look on both the stream channel, and the parent channel to see if the CLOSE_AFTER_RESPONSE flag
   * has been set. If so, return that promise.
   *
   * @param ctx
   * @param parent
   * @return
  ...
  private ChannelPromise shouldCloseAfter(ChannelHandlerContext ctx, Channel parent) {
    ChannelPromise closeAfterPromise =
        ctx.channel().attr(ConnectionCloseChannelAttributes.CLOSE_AFTER_RESPONSE).get();
    if (closeAfterPromise == null) {
  ...
  private void closeChannel(ChannelHandlerContext ctx, ChannelPromise promise) throws Exception {
    ...
    // 1. Check if already_closing flag on this stream channel. If there is, then success this
    // promise and return.
  ...
  /**
   * WARNING: Found the OkHttp client gets confused by this behaviour (it ends up putting itself in
   * a bad shutdown state after receiving the first goaway frame, and then dropping any inflight
   * responses but also timing out waiting for them).
   *
   * <p>And worried that other http/2 stacks may be similar, so for now we should NOT use this.
   *
   * <p>This is unfortunate, as FTL wanted this, and it is correct according to the spec.
   *
   * <p>See this code in okhttp where it drops response header frame if state is already shutdown:
  ...
  private void gracefullyWithDelay(EventExecutor executor, Channel parent, ChannelPromise promise) {
    ...
    executor.schedule(
        () -> {
          ...
          // Check that the client hasn't already closed the connection (due to the earlier goaway
          // we sent).
          if (parent.isActive()) {
            // NOTE - the netty Http2ConnectionHandler specifically does not send another goaway
            // when we call
            // channel.close() if one has already been sent .... so when we want more than one sent,
            // we need to do it
            ...
            Http2Exception h2e =
                new Http2Exception(
                    Http2Error.NO_ERROR, Http2Exception.ShutdownHint.GRACEFUL_SHUTDOWN);
