...
public class Http2ConnectionCloseHandler extends ChannelDuplexHandler {
    ...
    @Inject
    public Http2ConnectionCloseHandler(@Nullable Registry registry) {
        super();
    ...
    private void incrementCounter(ConnectionCloseType closeType, int port) {
        registry.counter(counterBaseId.withTag("close_type", closeType.name()).withTag("port", Integer.toString(port)).withTag("protocol", "http2")).increment();
    }
