...
/**
 * Copies any decoded HAProxyMessage into the channel attributes, and doesn't pass it any further
 * along the pipeline. Use in conjunction with HAProxyMessageDecoder if proxy protocol is enabled on
 * the ELB.
 */
public final class HAProxyMessageChannelHandler extends ChannelInboundHandlerAdapter {
  ...
  @VisibleForTesting static final Attrs.Key<Integer> HAPM_DEST_PORT = Attrs.newKey("hapm_port");
  ...
  public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
    if (msg instanceof HAProxyMessage) {
        ...
        out:
        {
          switch (hapm.proxiedProtocol()) {
            ...
            case TCP6:
              ...
              addr = inetAddr;
              // setting PPv2 explicitly because SourceAddressChannelHandler.ATTR_LOCAL_ADDR could
              // be PPv2 or not
              channel
                  .attr(SourceAddressChannelHandler.ATTR_PROXY_PROTOCOL_DESTINATION_ADDRESS)
                  .set(inetAddr);
      ...
      // Get the real client IP from the ProxyProtocol message sent by the ELB, and overwrite the
      // SourceAddress
      // channel attribute.
