...
public class HttpHeaderNamesCache {
  ...
  public HeaderName get(String name) {
    ...
    // caching the object in the first place (ie. the expensive operation we want to avoid by
    // caching
    // is the case-insensitive string comparisons).
    HeaderName hn = cache.get(name);
    if (hn == null) {
      // Here we're accepting that the isFull check is not happening atomically with the put, as we
      // don't mind
