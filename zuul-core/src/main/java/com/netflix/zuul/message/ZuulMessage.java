...
public interface ZuulMessage extends Cloneable {
  ...
  /**
   * Returns the headers for this message. They may be request or response headers, depending on the
   * underlying type of this object. For some messages, there may be no headers, such as with
  ...
  /**
   * Returns if this message has an attached body. For requests, this is typically an HTTP POST
   * body. For responses, this is typically the HTTP response.
   */
  boolean hasBody();

  /**
   * Declares that this message has a body. This method is automatically called when {@link
   * #bufferBodyContents} is invoked.
   */
  ...
  /**
   * Appends an HTTP content chunk to this message. Callers should be careful not to add multiple
   * chunks that implement {@link LastHttpContent}.
   *
  ...
  /**
   * Returns the HTTP content chunks that are part of this message. Callers should avoid retaining
   * the return value, as the contents may change with subsequent body mutations.
   */
  ...
  /**
   * Clears the content chunks of this body, calling {@code release()} in the process. Users SHOULD
   * call this method when the body content is no longer needed.
   */
