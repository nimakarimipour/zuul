...
/**
 * This class manages the directory polling for changes and new Groovy filters. Polling interval and
 * directories are specified in the initialization of the class, and a poller will check for changes
 * and additions.
 *
 * @author Mikey Cohen Date: 12/7/11 Time: 12:09 PM
 */
@Singleton
public class FilterFileManager {
  ...
  public void init() throws Exception {
    ...
    LOG.warn(
        "Finished loading all zuul filters. Duration = {} ms.",
  ...
  void startPoller() {
    poller =
        new Thread("GroovyFilterFileManagerPoller") {
          {
            setDaemon(true);
          }
      ...
      tasks.add(
          () -> {
            try {
              ...
              LOG.error(
                  "Error loading groovy filter from disk! file = {}", String.valueOf(file), e);
              return false;
            }
          });
    }
