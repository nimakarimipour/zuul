 ...
 *
 * @author Mikey Cohen Date: 2/3/12 Time: 3:25 PM
 */
public class StatsManager {
  ...
  private static final Pattern HOST_PATTERN =
      Pattern.compile(
          "(?:(.+)\\.amazonaws\\.com)|((?:\\d{1,3}\\.?){4})|(ip-\\d+-\\d+-\\d+-\\d+)|"
              + "(?:(.+)\\.nflxvideo\\.net)|(?:(.+)\\.llnwd\\.net)|(?:(.+)\\.nflximg\\.com)");

  @VisibleForTesting static final String HOST_HEADER = "host";

  private static final String X_FORWARDED_FOR_HEADER = "x-forwarded-for";

  @VisibleForTesting static final String X_FORWARDED_PROTO_HEADER = "x-forwarded-proto";
  ...
  /**
   * Collects counts statistics about the request: client ip address from the x-forwarded-for
   * header; ipv4 or ipv6 and host name from the host header;
   *
   * @param req
   */
  ...
  protected void incrementNamedCountingMonitor(
      String name, ConcurrentMap<String, NamedCountingMonitor> map) {
  ...
  /**
   * collects and increments counts of status code, route/status code and statuc_code bucket, eg 2xx
   * 3xx 4xx 5xx
   *
   * @param route
   * @param statusCode
