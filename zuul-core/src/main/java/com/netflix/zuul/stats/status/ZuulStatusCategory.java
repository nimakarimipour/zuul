 ...
 *
 * <p>As some of the origin servers won't/can't return correct HTTP status codes in responses, we
 * use set an StatusCategory attribute to distinguish the main statuses that we care about from
 * Zuul's perspective.
 *
 * <p>These status categories are split into 2 groups:
 *
 * <p>SUCCESS | FAILURE
 *
 * <p>each of which can have a narrower definition, eg:
 *
 * <p>FAILURE_THROTTLED FAILURE_ORIGIN etc...
 *
 * <p>which _should_ also be subdivided with one of:
 *
 * <p>ORIGIN CLIENT LOCAL
 */
public enum ZuulStatusCategory implements StatusCategory {
  ...
  SUCCESS_LOCAL_NOTSET(
      ...
      4), // This is set on the SessionContext as the default value.
  ...
  FAILURE_LOCAL_THROTTLED_ORIGIN_SERVER_MAXCONN(
      ZuulStatusCategoryGroup.FAILURE,
      7), // NIWS client throttling based on max connections per origin server.
  FAILURE_LOCAL_THROTTLED_ORIGIN_CONCURRENCY(
      ZuulStatusCategoryGroup.FAILURE,
      8), // when zuul throttles for a vip because concurrency is too high.
  ...
  FAILURE_CLIENT_CANCELLED(
      ZuulStatusCategoryGroup.FAILURE,
      13), // client abandoned/closed the connection before origin responded.
  ...
  FAILURE_ORIGIN_THROTTLED(
      ZuulStatusCategoryGroup.FAILURE, 6), // Throttled by origin by returning 503
  FAILURE_ORIGIN_NO_SERVERS(
      ZuulStatusCategoryGroup.FAILURE, 14), // No UP origin servers available in Discovery.
