...
public class BasicNettyOrigin implements NettyOrigin {
    ...
    @Override
    public Promise<PooledConnection> connectToOrigin(HttpRequestMessage zuulReq, EventLoop eventLoop, int attemptNumber, @Nullable CurrentPassport passport, AtomicReference<Server> chosenServer, AtomicReference<? super InetAddress> chosenHostAddr) {
    ...
    @Override
    public void recordFinalError(HttpRequestMessage requestMsg, @Nullable Throwable throwable) {
        if (throwable == null) {
            return;
        }
