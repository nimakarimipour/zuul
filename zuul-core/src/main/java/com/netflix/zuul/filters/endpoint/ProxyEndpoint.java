...
public class ProxyEndpoint extends SyncZuulFilterAdapter<HttpRequestMessage, HttpResponseMessage> implements GenericFutureListener<Future<PooledConnection>> {
    ...
    @Override
    @Nullable
    public HttpContent processContentChunk(@Nullable final ZuulMessage zuulReq, final HttpContent chunk) {
        if (originConn != null) {
    ...
    private byte[] preCacheBodyForRetryingSslRequests() {
        ...
        if (// only cache requests if already buffered
        ENABLE_CACHING_SSL_BODIES.get() && origin != null && origin.getClientConfig().get(IClientConfigKey.Keys.IsSecure, false) && zuulRequest.hasCompleteBody()) {
    ...
    private HttpResponseMessage buildZuulHttpResponse(final HttpResponse httpResponse, final StatusCategory statusCategory, @Nullable final Throwable ex) {
        startedSendingResponseToClient = true;
