 ...
 * but NOT for each attempt/retry. All the retry attempts for a given HTTP/1.1 request proxied share
 * the same EdgeProxyEndpoint instance Created by saroskar on 5/31/17.
 */
...
public class ProxyEndpoint extends SyncZuulFilterAdapter<HttpRequestMessage, HttpResponseMessage>
  ...
  private static final String ZUUL_ORIGIN_ATTEMPT_IPADDR_MAP_KEY =
      "_zuul_origin_attempt_ipaddr_map";
  ...
  public static final Set<String> IDEMPOTENT_HTTP_METHODS =
      Sets.newHashSet("GET", "HEAD", "OPTIONS");
  ...
  /**
   * Indicates how long Zuul should remember throttle events for an origin. As of this writing,
   * throttling is used to decide to cache request bodies.
   */
  private static final CachedDynamicLongProperty THROTTLE_MEMORY_SECONDS =
      new CachedDynamicLongProperty(
  ...
  private static final Counter NO_RETRY_RESP_STARTED =
      SpectatorUtils.newCounter("zuul.no.retry", "resp_started");
  ...
  public HttpResponseMessage apply(final HttpRequestMessage input) {
    ...
    try {
      ...
      // Doesn't return origin response to caller, calls invokeNext() internally in response filter
      // chain
      return null;
    } catch (Exception ex) {
      handleError(ex);
  ...
  public void invokeNext(final HttpContent chunk) {
    try {
      ...
      ByteBufUtil.touch(
          chunk, "ProxyEndpoint exception processing chunk from origin, request: ", zuulRequest);
      unlinkFromOrigin();
  ...
  private void filterResponseChunk(final HttpContent chunk) {
    if (context.isCancelled() || !channelCtx.channel().isActive()) {
      SpectatorUtils.newCounter(
              "zuul.origin.strayChunk", origin == null ? "none" : origin.getName().getMetricId())
          .increment();
      unlinkFromOrigin();
  ...
  private void storeAndLogOriginRequestInfo() {
    ...
    // These two maps appear to be almost the same but are slightly different.   Also, the types in
    // the map don't
    // match exactly what needs to happen, so this is more of a To-Do.
    // ZUUL_ORIGIN_ATTEMPT_IPADDR_MAP_KEY is
    // supposed to be the mapping of IP addresses of the server.  This is (AFAICT) only used for
    // logging.   It is
    // an IP address semantically, but a String here.   The two should be swapped.
    // ZUUL_ORIGIN_CHOSEN_HOST_ADDR_MAP_KEY is almost always an IP address, but may some times be a
    // hostname in
    ...
    // the chosen server can be null in the case of a timeout exception that skips acquiring a new
    // origin connection
    ...
    if (chosenHostAddr.get() != null) {
      ...
      eventProps.put(
          CommonContextKeys.ZUUL_ORIGIN_CHOSEN_HOST_ADDR_MAP_KEY.name(), attemptToChosenHostMap);
  ...
  private void proxyRequestToOrigin() {
    ...
    try {
      ...
      // We pass this AtomicReference<Server> here and the origin impl will assign the chosen server
      // to it.
      promise =
          origin.connectToOrigin(
              zuulRequest,
              channelCtx.channel().eventLoop(),
              attemptNum,
              passport,
              chosenServer,
              chosenHostAddr);
    ...
    try {
      methodBinding.bind(
          () -> {
            ...
            /**
             * TODO(argha-c): This reliance on mutable update of the `chosenServer` must be
             * improved.
             *
             * @see DiscoveryResult.EMPTY indicates that the loadbalancer found no available
             *     servers.
             */
  ...
  private static void writeBufferedBodyContent(
      final HttpRequestMessage zuulRequest, final Channel channel) {
    zuulRequest
        .getBodyContents()
        .forEach(
            (chunk) -> {
  ...
  private void processErrorFromOrigin(final Throwable ex, final Channel origCh) {
    try {
      ...
      final ClientException niwsEx =
          new ClientException(ClientException.ErrorType.valueOf(err.getClientErrorType().name()));
      ...
      if ((isBelowRetryLimit()) && (isRetryable(err))) {
        ...
        // Record the exception in context. An error filter should later run which can translate
        // this into an
  ...
  private void handleError(final Throwable cause) {
    final ZuulException ze =
        (cause instanceof ZuulException)
            ? (ZuulException) cause
  ...
  /**
   * Request is replayable on a different origin IFF A) we have not started to send response back to
   * the client AND B) we have not lost any of its body chunks
   */
  ...
  protected void handleOriginSuccessResponse(
      final HttpResponse originResponse, DiscoveryResult chosenServer) {
  ...
  private HttpResponseMessage buildZuulHttpResponse(
    ...
    final HttpResponseMessage zuulResponse =
        new HttpResponseMessageImpl(zuulCtx, zuulRequest, respStatus);
    ...
    // Store this original response info for future reference (ie. for metrics and access logging
    // purposes).
    ...
    // Request was a success even if server may have responded with an error code 5XX, except for
    // 503.
    if (originConn != null) {
      if (statusCategory == ZuulStatusCategory.FAILURE_ORIGIN_THROTTLED) {
        ...
        origin.onRequestExecutionSuccess(
            zuulRequest, zuulResponse, originConn.getServer(), attemptNum);
      }
    }
  ...
  protected void handleOriginNonSuccessResponse(
    ...
    origin.onRequestExceptionWithServer(
        zuulRequest, chosenServer, attemptNum, new ClientException(niwsErrorType));
    ...
    if ((isBelowRetryLimit()) && (isRetryable5xxResponse(zuulRequest, originResponse))) {
      ...
      // otherwise when the body is read by netty (in writeBufferedBodyContent) the body will appear
      // empty
  ...
  public boolean isRetryable5xxResponse(
    ...
    if (isRequestReplayable()) {
      ...
      // Retry if this is an idempotent http method AND status code was retriable for idempotent
      // methods.
  ...
  protected NettyOrigin getOrigin(HttpRequestMessage request) {
    ...
    if (Strings.isNullOrEmpty(primaryRoute)) {
      // If no vip selected, leave origin null, then later the handleNoOriginSelected() method will
      // be invoked.
      return null;
    }

    // make sure the restClientName will never be a raw VIP in cases where it's the fallback for
    // another route assignment
    ...
    if (overrideOriginName != null) {
      ...
      origin =
          getOrCreateOrigin(originManager, overrideOriginName, request.reconstructURI(), context);
    ...
    // Update the routeVip on context to show the actual raw VIP from the clientConfig of the chosen
    // Origin.
    if (origin != null) {
      context.set(
  ...
  private NettyOrigin getOrCreateOrigin(
    ...
    if (origin == null) {
      // If no pre-registered and configured RestClient found for this VIP, then register one using
      // default NIWS
      // properties.
      LOG.warn(
  ...
  private void verifyOrigin(
    ...
    if (primaryOrigin == null) {
      // If no origin found then add specific error-cause metric tag, and throw an exception with
      // 404 status.
      ...
      ZuulException ze =
          new ZuulException(
              "No origin found for request. name="
                  + restClientName
                  + ", uri="
