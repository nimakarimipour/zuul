 ...
 *
 * @author Mikey Cohen Date: 10/27/11 Time: 3:03 PM
 */
public interface ZuulFilter<I extends ZuulMessage, O extends ZuulMessage> extends ShouldFilter<I> {
  ...
  /**
   * to classify a filter by type. Standard types in Zuul are "in" for pre-routing filtering, "end"
   * for routing to an origin, "out" for post-routing filters.
   *
   * @return FilterType
   */
  ...
  /**
   * Whether this filter's shouldFilter() method should be checked, and apply() called, even if
   * SessionContext.stopFilterProcessing has been set.
   *
   * @return boolean
   */
  ...
  /**
   * if shouldFilter() is true, this method will be invoked. this method is the core method of a
   * ZuulFilter
   */
  ...
  /**
   * Filter indicates it needs to read and buffer whole body before it can operate on the messages
   * by returning true. The decision can be made at runtime, looking at the request type. For
   * example if the incoming message is a MSL message MSL decryption filter can return true here to
   * buffer whole MSL message before it tries to decrypt it.
   *
