...
/**
 * This is an abstract filter that will route requests that match the patternMatches() method to a
 * debug Eureka "VIP" or host specified by zuul.debug.vip or zuul.debug.host.
 *
 * @author Mikey Cohen Date: 6/27/12 Time: 12:54 PM
 */
@Filter(order = 99, type = FilterType.INBOUND)
public class SurgicalDebugFilter extends HttpInboundSyncFilter {

  /**
   * Returning true by the pattern or logic implemented in this method will route the request to the
   * specified origin
   *
   * <p>Override this method when using this filter to add your own pattern matching logic.
   *
  ...
  public boolean shouldFilter(HttpRequestMessage request) {
    ...
    String isSurgicalFilterRequest =
        request.getHeaders().getFirst(ZuulHeaders.X_ZUUL_SURGICAL_FILTER);
  ...
  public HttpRequestMessage apply(HttpRequestMessage request) {
    ...
    DynamicStringProperty routeHost =
        new DynamicStringProperty(ZuulConstants.ZUUL_DEBUG_HOST, null);
