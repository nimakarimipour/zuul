...
public class OriginResponseReceiver extends ChannelDuplexHandler {
  ...
  public void userEventTriggered(ChannelHandlerContext ctx, Object evt) throws Exception {
    if (evt instanceof CompleteEvent) {
      ...
      if (edgeProxy != null) {
        LOG.error(
            "Origin request received IDLE event: {}",
            ChannelUtils.channelInfoForLogging(ctx.channel()));
  ...
  private static String pathAndQueryString(HttpRequestMessage request) {
    // parsing the params cleans up any empty/null params using the logic of the HttpQueryParams
    // class
  ...
  private void writeInternal(ChannelHandlerContext ctx, Object msg, ChannelPromise promise)
      throws Exception {
    ...
    if (msg instanceof HttpRequestMessage) {
      promise.addListener(
          (future) -> {
            if (!future.isSuccess()) {
              Throwable cause =
                  ctx.channel().attr(SSL_HANDSHAKE_UNSUCCESS_FROM_ORIGIN_THROWABLE).get();
              if (cause != null) {
  ...
  public void channelInactive(ChannelHandlerContext ctx) throws Exception {
    if (edgeProxy != null) {
      LOG.debug(
          "Origin channel inactive. channel-info={}",
          ChannelUtils.channelInfoForLogging(ctx.channel()));
      OriginConnectException ex =
          new OriginConnectException("Origin server inactive", RESET_CONNECTION);
