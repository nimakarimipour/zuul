...
public class ClientResponseWriter extends ChannelInboundHandlerAdapter {
      ...
      if (channel.isActive()) {
        ...
        if (!ClientRequestReceiver.isLastContentReceivedForChannel(channel)) {
          ...
          StatusCategory status =
              StatusCategoryUtils.getStatusCategory(
                  ClientRequestReceiver.getRequestFromChannel(channel));
          if (ZuulStatusCategory.FAILURE_CLIENT_TIMEOUT.equals(status)) {
            // If the request timed-out while being read, then there won't have been any
            // LastContent, but thats ok because the connection will have to be discarded anyway.
          } else {
  ...
  private static void writeBufferedBodyContent(
      final HttpResponseMessage zuulResponse, final Channel channel) {
  ...
  private HttpResponse buildHttpResponse(final HttpResponseMessage zuulResp) {
    ...
    final DefaultHttpResponse nativeResponse =
    ...
    // Netty does not automatically add Content-Length or Transfer-Encoding: chunked. So we add here
    // if missing.
    if (!HttpUtil.isContentLengthSet(nativeResponse)
        && !HttpUtil.isTransferEncodingChunked(nativeResponse)) {
  ...
  public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
    ...
    if (isHandlingRequest && !startedSendingResponseToClient && ctx.channel().isActive()) {
      final HttpResponse httpResponse =
          new DefaultFullHttpResponse(HTTP_1_1, HttpResponseStatus.valueOf(status));
