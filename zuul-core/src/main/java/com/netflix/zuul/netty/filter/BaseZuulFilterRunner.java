 ...
 * member variables Created by saroskar on 5/18/17.
 */
@ThreadSafe
public abstract class BaseZuulFilterRunner<I extends ZuulMessage, O extends ZuulMessage>
  ...
  public static final ChannelHandlerContext getChannelHandlerContext(final ZuulMessage mesg) {
    return (ChannelHandlerContext)
        checkNotNull(
            mesg.getContext().get(NETTY_SERVER_CHANNEL_HANDLER_CONTEXT), "channel handler context");
  }
      ...
      try (TaskCloseable ignored =
        ...
        if (!channelHandlerContext.channel().isActive()) {
          ...
          SpectatorUtils.newCounter(
                  "zuul.filterChain.chunk.hanging", zuulMesg.getClass().getSimpleName())
              .increment();
        } else {
    ...
    if (nextStage != null) {
      ...
      try (TaskCloseable ignored =
        ...
        if (!channelHandlerContext.channel().isActive()) {
          ...
          zuulMesg.disposeBufferedBody();
          SpectatorUtils.newCounter(
                  "zuul.filterChain.message.hanging", zuulMesg.getClass().getSimpleName())
              .increment();
        } else {
  ...
  protected final O filter(final ZuulFilter<I, O> filter, final I inMesg) {
    ...
    try (TaskCloseable ignored = traceTask(filter, f -> f.filterName() + ".filter")) {
      ...
      if (filter.filterType() == INBOUND && inMesg.getContext().shouldSendErrorResponse()) {
        // Pass request down the pipeline, all the way to error endpoint if error response needs to
        // be generated
  ...
  protected O handleFilterException(
      final I inMesg, final ZuulFilter<I, O> filter, final Throwable ex) {
  ...
  protected void recordFilterError(
      final I inMesg, final ZuulFilter<I, O> filter, final Throwable t) {
    // Add a log statement for this exception.
    final String errorMsg =
        "Filter Exception: filter="
            + filter.filterName()
            + ", request-info="
            + inMesg.getInfoForLogging()
            + ", msg="
    ...
    // the chain.
    final SessionContext zuulCtx = inMesg.getContext();
    if (zuulCtx != null && zuulCtx.getFilterErrors() != null) {
      zuulCtx
          .getFilterErrors()
          .add(new FilterError(filter.filterName(), filter.filterType().toString(), t));
    }
    if (zuulCtx != null && zuulCtx.debugRouting()) {
      Debug.addRoutingDebug(
          zuulCtx,
  ...
  protected void handleException(
      final ZuulMessage zuulMesg, final String filterName, final Exception ex) {
    ...
    final String errMesg =
        "Error with filter: " + filterName + ", path: " + path + ", method: " + method;
