...
/**
 * TODO: Change this class to be an instance per-port. So that then the configuration can be
 * different per-port, which is need for the combined FTL/Cloud clusters.
 *
 * <p>User: michaels@netflix.com Date: 3/6/17 Time: 12:36 PM
 */
public class ClientConnectionsShutdown {
  ...
  public ClientConnectionsShutdown(
      ChannelGroup channels, EventExecutor executor, EurekaClient discoveryClient) {
    ...
    //        DynamicBooleanProperty DEBUG_SHUTDOWN = new
    // DynamicBooleanProperty("server.outofservice.connections.shutdown.debug", false);
  ...
  private void initDiscoveryListener() {
    this.discoveryClient.registerEventListener(
        event -> {
  ...
  public void gracefullyShutdownClientChannels() {
    ...
    try {
      ...
      for (Channel channel : channels) {
        ...
        // TODO(carl-mastrangelo): remove closePromise, since I don't think it's needed.  Need to
        // verify.
