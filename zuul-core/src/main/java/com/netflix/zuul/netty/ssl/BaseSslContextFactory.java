...
/** User: michaels@netflix.com Date: 3/4/16 Time: 4:00 PM */
public class BaseSslContextFactory implements SslContextFactory {
  ...
  public SslContextBuilder createBuilderForServer() {
    try {
      ...
      if (serverSslConfig.getClientAuth() != null
          && trustedCerts != null
  ...
  /**
   * This function is meant to call the correct overload of {@code SslContextBuilder.forServer()}.
   * It should not apply any other customization.
   */
  @ForOverride
  ...
  public void configureOpenSslStatsMetrics(SslContext sslContext, String sslContextId) {
    ...
    if (sslContext instanceof ReferenceCountedOpenSslContext) {
      OpenSslSessionStats stats =
          ((ReferenceCountedOpenSslContext) sslContext).sessionContext().stats();
      ...
      openSslStatGauge(
          stats, sslContextId, "accept_renegotiate", OpenSslSessionStats::acceptRenegotiate);
      ...
      openSslStatGauge(
          stats, sslContextId, "connect_renegotiate", OpenSslSessionStats::connectRenegotiate);
      ...
      openSslStatGauge(
          stats, sslContextId, "ticket_key_renew", OpenSslSessionStats::ticketKeyRenew);
      openSslStatGauge(
  ...
  protected ArrayList<X509Certificate> getTrustedX509Certificates()
    ...
    // Add the certificates from the JKS truststore - ie. the CA's of the client cert that peer
    // Zuul's will use.
    if (serverSslConfig.getClientAuth() == ClientAuth.REQUIRE
      ...
      if (serverSslConfig.getClientAuthTrustStorePassword() != null) {
        trustStorePwdBytes =
            Base64.getDecoder().decode(serverSslConfig.getClientAuthTrustStorePassword());
      } else if (serverSslConfig.getClientAuthTrustStorePasswordFile() != null) {
        trustStorePwdBytes =
            Files.readAllBytes(serverSslConfig.getClientAuthTrustStorePasswordFile().toPath());
      } else {
