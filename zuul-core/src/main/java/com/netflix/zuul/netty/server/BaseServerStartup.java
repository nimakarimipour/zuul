...
public abstract class BaseServerStartup {
  ...
  public void init() throws Exception {
    ...
    clientConnectionsShutdown =
        new ClientConnectionsShutdown(
            clientChannels, GlobalEventExecutor.INSTANCE, discoveryClient);
  ...
  @ForOverride
  protected Map<NamedSocketAddress, ChannelInitializer<?>> chooseAddrsAndChannels(
      ChannelGroup clientChannels) {
    @SuppressWarnings(
        "unchecked") // Channel init map has the wrong generics and we can't fix without api
    // breakage.
  ...
  protected void addChannelDependencies(
      ...
      @SuppressWarnings("unused")
          String listenAddressName) { // listenAddressName is used by subclasses
    ...
    final Counter httpRequestReadTimeoutCounter =
        registry.counter("server.http.request.read.timeout");
    channelDeps.set(
        ZuulDependencyKeys.httpRequestReadTimeoutCounter, httpRequestReadTimeoutCounter);
    ...
    channelDeps.set(
        ZuulDependencyKeys.sslClientCertCheckChannelHandlerProvider,
        new NullChannelHandlerProvider());
    channelDeps.set(
   ...
   * "server.${addrName}.${propertySuffix}". If none found, then looks for a server-wide property of
   * the form - "server.${propertySuffix}". If that is also not found, then returns the specified
   * default value.
   */
  public static int chooseIntChannelProperty(
      String listenAddressName, String propertySuffix, int defaultValue) {
  ...
  public static boolean chooseBooleanChannelProperty(
    ...
    Boolean value =
        new ChainedDynamicProperty.DynamicBooleanPropertyThatSupportsNull(
                listenAddressPropertyName, null)
            .get();
    if (value == null) {
      value =
          new DynamicBooleanProperty(globalPropertyName, defaultValue)
              .getDynamicProperty()
              .getBoolean();
      if (value == null) {
        value = defaultValue;
  ...
  public static ChannelConfig defaultChannelConfig(String listenAddressName) {
    ...
    config.add(
        new ChannelConfigValue<>(
            CommonChannelConfigKeys.maxConnections,
            chooseIntChannelProperty(
                listenAddressName,
                "connection.max",
                CommonChannelConfigKeys.maxConnections.defaultValue())));
    config.add(
        new ChannelConfigValue<>(
    ...
    config.add(
        new ChannelConfigValue<>(CommonChannelConfigKeys.idleTimeout, connectionIdleTimeout));
    config.add(
        new ChannelConfigValue<>(
  ...
  public static void addHttp2DefaultConfig(ChannelConfig config, String listenAddressName) {
    ...
    // Override this to a lower value, as we'll be using ELB TCP listeners for h2, and therefore the
    // connection
    ...
    config.add(
        new ChannelConfigValue<>(
            ...
            chooseBooleanChannelProperty(
                listenAddressName, "connection.close.graceful.delayed.allow", true)));
    config.add(
        new ChannelConfigValue<>(
            CommonChannelConfigKeys.http2SwallowUnknownExceptionsOnConnClose,
            chooseBooleanChannelProperty(
                listenAddressName, "connection.close.swallow.unknown.exceptions", false)));
  }
