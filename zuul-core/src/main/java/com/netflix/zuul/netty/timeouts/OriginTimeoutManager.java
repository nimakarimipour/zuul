...
public class OriginTimeoutManager {
  ...
  /**
   * Derives the read timeout from the configuration. This implementation prefers the longer of
   * either the origin timeout or the request timeout.
   *
   * <p>This method can also be used to validate timeout and deadline boundaries and throw
   * exceptions as needed. If extending this method to do validation, you should extend {@link
   * com.netflix.zuul.exception.OutboundException} and set the appropriate {@link
   * com.netflix.zuul.exception.ErrorType}.
   *
  ...
  protected IClientConfig getRequestClientConfig(HttpRequestMessage zuulRequest) {
    IClientConfig overriddenClientConfig =
        zuulRequest.getContext().get(CommonContextKeys.REST_CLIENT_CONFIG);
