...
public class ZuulEndPointRunner
  ...
  public void filter(final HttpRequestMessage zuulReq) {
    ...
    try (TaskCloseable ignored =
        PerfMark.traceTask(this, s -> s.getClass().getSimpleName() + ".filter")) {
      Preconditions.checkNotNull(zuulReq, "input message");
      addPerfMarkTags(zuulReq);

      final ZuulFilter<HttpRequestMessage, HttpResponseMessage> endpoint =
          getEndpoint(endpointName, zuulReq);
      logger.debug(
          "Got endpoint {}, UUID {}", endpoint.filterName(), zuulReq.getContext().getUUID());
  ...
  protected void resume(final HttpResponseMessage zuulMesg) {
    try (TaskCloseable ignored =
        PerfMark.traceTask(this, s -> s.getClass().getSimpleName() + ".resume")) {
  ...
  protected String getEndPointName(final SessionContext zuulCtx) {
    if (zuulCtx.shouldSendErrorResponse()) {
      ...
      return (Strings.isNullOrEmpty(errEndPointName))
          ? DEFAULT_ERROR_ENDPOINT.get()
          : errEndPointName;
    } else {
  ...
  protected ZuulFilter<HttpRequestMessage, HttpResponseMessage> getEndpoint(
    ...
    final Endpoint<HttpRequestMessage, HttpResponseMessage> filter =
        getEndpointFilter(endpointName);
    if (filter == null) {
  ...
  protected static final ZuulFilter<HttpRequestMessage, HttpResponseMessage>
