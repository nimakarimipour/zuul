 ...
 * This URL handler MUST BE accessible ONLY from RFC 1918 private internal network space (10.0.0.0
 * or 172.16.0.0) to guarantee that external applications/agents cannot push messages to your
 * client. In AWS this can typically be achieved using correctly configured security groups.
 *
 * <p>Author: Susheel Aroskar Date: 5/14/18
 */
@Singleton
  ...
  protected void channelRead0(final ChannelHandlerContext ctx, final FullHttpRequest request)
    ...
    if (path.endsWith("/push")) {
      ...
      if (body.readableBytes() <= 0) {
        ...
        // Because we are not passing the body to the pushConn (who would normally handle
        // destroying),
