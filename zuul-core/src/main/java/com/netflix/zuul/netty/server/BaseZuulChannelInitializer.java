...
public abstract class BaseZuulChannelInitializer extends ChannelInitializer<Channel> {
  ...
  public static final AttributeKey<ChannelConfig> ATTR_CHANNEL_CONFIG =
      AttributeKey.newInstance("channel_config");
  ...
  /**
   * The port that the server intends to listen on. Subclasses should NOT use this field, as it may
   * not be set, and may differ from the actual listening port. For example:
   *
   * <ul>
  ...
  private BaseZuulChannelInitializer(
    ...
    this.maxConnectionsHandler =
        new MaxInboundConnectionsHandler(registry, metricId, maxConnections);
    ...
    this.stripInboundProxyHeadersHandler =
        new StripUntrustedProxyHeadersHandler(allowProxyHeadersWhen);

    this.rateLimitingChannelHandler =
        channelDependencies.get(ZuulDependencyKeys.rateLimitingChannelHandlerProvider).get();

    this.sslClientCertCheckChannelHandler =
        channelDependencies.get(ZuulDependencyKeys.sslClientCertCheckChannelHandlerProvider).get();
  ...
  protected HttpServerCodec createHttpServerCodec() {
    return new HttpServerCodec(
        MAX_INITIAL_LINE_LENGTH.get(), MAX_HEADER_SIZE.get(), MAX_CHUNK_SIZE.get(), false);
  }
  ...
  protected <T extends ZuulMessage, R extends ZuulMessage>
  ...
  public <T extends ZuulMessage> ZuulFilter<T, T>[] getFilters(
      ...
      if (zuulFilters == null) {
        throw new IllegalStateException("zuulFilters is null");
      }
