...
/** User: michaels@netflix.com Date: 2/28/17 Time: 5:41 PM */
...
public class PassportLoggingHandler extends ChannelInboundHandlerAdapter {
  ...
  private static final CachedDynamicLongProperty WARN_REQ_PROCESSING_TIME_NS =
      new CachedDynamicLongProperty(
          "zuul.passport.log.request.time.threshold", 1000 * 1000 * 1000); // 1000 ms
  private static final CachedDynamicLongProperty WARN_RESP_PROCESSING_TIME_NS =
      new CachedDynamicLongProperty(
          "zuul.passport.log.response.time.threshold", 1000 * 1000 * 1000); // 1000 ms
  ...
  private void logPassport(Channel channel) {
    ...
    if (LOG.isInfoEnabled()) {
      ...
      StartAndEnd inReqToOutResp =
          passport.findFirstStartAndLastEndStates(
      ...
      StartAndEnd inRespToOutResp =
          passport.findLastStartAndFirstEndStates(
