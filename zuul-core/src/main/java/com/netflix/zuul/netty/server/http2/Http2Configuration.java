...
public class Http2Configuration {
    ...
    public static SslContext configureSSL(SslContextFactory sslContextFactory, String metricId) {
        ...
        ApplicationProtocolConfig apn = new // NO_ADVERTISE is currently the only mode supported by both OpenSsl and JDK providers.
        ApplicationProtocolConfig(// NO_ADVERTISE is currently the only mode supported by both OpenSsl and JDK providers.
        ApplicationProtocolConfig.Protocol.ALPN, // ACCEPT is currently the only mode supported by both OpenSsl and JDK providers.
        ApplicationProtocolConfig.SelectorFailureBehavior.NO_ADVERTISE, ApplicationProtocolConfig.SelectedListenerFailureBehavior.ACCEPT, supportedProtocols);
    ...
    public static SslContext configureSSLWithH2Disabled(SslContextFactory sslContextFactory, String host) {
        ...
        ApplicationProtocolConfig(// NO_ADVERTISE is currently the only mode supported by both OpenSsl and JDK providers.
        ApplicationProtocolConfig.Protocol.ALPN, // ACCEPT is currently the only mode supported by both OpenSsl and JDK providers.
        ApplicationProtocolConfig.SelectorFailureBehavior.NO_ADVERTISE, ApplicationProtocolConfig.SelectedListenerFailureBehavior.ACCEPT, ApplicationProtocolNames.HTTP_1_1);
        final SslContext sslContext;
        try {
