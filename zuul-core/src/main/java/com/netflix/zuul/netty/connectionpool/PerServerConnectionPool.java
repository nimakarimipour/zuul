...
public class PerServerConnectionPool implements IConnectionPool {
    ...
    protected void tryMakingNewConnection(EventLoop eventLoop, Promise<PooledConnection> promise, CurrentPassport passport, AtomicReference<? super InetAddress> selectedHostAddr) {
        ...
        if (maxConnectionsPerHost != -1 && openAndOpeningConnectionCount >= maxConnectionsPerHost) {
            ...
            LOG.warn("Unable to create new connection because at MaxConnectionsPerHost! " + "maxConnectionsPerHost=" + maxConnectionsPerHost + ", connectionsPerHost=" + openAndOpeningConnectionCount + ", host=" + server.getServerId() + "origin=" + config.getOriginName());
            return;
        }
        try {
            ...
            if (cf.isDone()) {
                ...
                cf.addListener(future -> {
                    try {
                        ...
                        LOG.warn("Error creating new connection! " + "origin=" + config.getOriginName() + ", host=" + server.getServerId());
                    }
                });
            }
        } catch (Throwable e) {
