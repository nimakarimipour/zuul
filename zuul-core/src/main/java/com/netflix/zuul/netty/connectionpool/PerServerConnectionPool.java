...
public class PerServerConnectionPool implements IConnectionPool {
  ...
  protected void tryMakingNewConnection(
    ...
    int openAndOpeningConnectionCount =
        server.getOpenConnectionsCount() + connCreationsInProgress.get();
  ...
  protected ChannelFuture connectToServer(
      EventLoop eventLoop, CurrentPassport passport, SocketAddress serverAddr) {
