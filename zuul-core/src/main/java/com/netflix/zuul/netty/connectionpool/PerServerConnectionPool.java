...
public class PerServerConnectionPool implements IConnectionPool {
    ...
    public PerServerConnectionPool(ServerStats stats, InstanceInfo instanceInfo, SocketAddress serverAddr, @Nullable NettyClientConnectionFactory connectionFactory, PooledConnectionFactory pooledConnectionFactory, ConnectionPoolConfig config, IClientConfig niwsClientConfig, Counter createNewConnCounter, Counter createConnSucceededCounter, Counter createConnFailedCounter, Counter requestConnCounter, Counter reuseConnCounter, Counter connTakenFromPoolIsNotOpen, Counter maxConnsPerHostExceededCounter, Timer connEstablishTimer, AtomicInteger connsInPool, AtomicInteger connsInUse) {
        this.stats = stats;
     ...
     */
    private void onAcquire(final PooledConnection conn, @Nullable CurrentPassport passport) {
    ...
    @Override
    public Promise<PooledConnection> acquire(EventLoop eventLoop, @Nullable CurrentPassport passport, AtomicReference<? super InetAddress> selectedHostAddr) {
    ...
    protected void tryMakingNewConnection(EventLoop eventLoop, Promise<PooledConnection> promise, @Nullable CurrentPassport passport, AtomicReference<? super InetAddress> selectedHostAddr) {
        ...
        if (maxConnectionsPerHost != -1 && openAndOpeningConnectionCount >= maxConnectionsPerHost) {
            ...
            promise.setFailure(new OriginConnectException("maxConnectionsPerHost=" + maxConnectionsPerHost + ", connectionsPerHost=" + openAndOpeningConnectionCount, OutboundErrorType.ORIGIN_SERVER_MAX_CONNS));
            LOG.warn("Unable to create new connection because at MaxConnectionsPerHost! " + "maxConnectionsPerHost=" + maxConnectionsPerHost + ", connectionsPerHost=" + openAndOpeningConnectionCount + ", host=" + instanceInfo.getId() + "origin=" + config.getOriginName());
            return;
        }
        try {
            ...
            if (cf.isDone()) {
                ...
                cf.addListener(future -> {
                    try {
                        ...
                        LOG.warn("Error creating new connection! " + "origin=" + config.getOriginName() + ", host=" + instanceInfo.getId());
                    }
                });
            }
        } catch (Throwable e) {
