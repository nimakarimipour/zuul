...
public class DefaultClientChannelManager implements ClientChannelManager {
  ...
  public DefaultClientChannelManager(
    ...
    this.createConnSucceededCounter =
        SpectatorUtils.newCounter(METRIC_PREFIX + "_create_success", metricId);
    this.createConnFailedCounter =
        SpectatorUtils.newCounter(METRIC_PREFIX + "_create_fail", metricId);
    ...
    this.alreadyClosedCounter =
        SpectatorUtils.newCounter(METRIC_PREFIX + "_alreadyClosed", metricId);
    this.connTakenFromPoolIsNotOpen =
        SpectatorUtils.newCounter(METRIC_PREFIX + "_fromPoolIsClosed", metricId);
    this.maxConnsPerHostExceededCounter =
        SpectatorUtils.newCounter(METRIC_PREFIX + "_maxConnsPerHostExceeded", metricId);
    this.closeWrtBusyConnCounter =
        SpectatorUtils.newCounter(METRIC_PREFIX + "_closeWrtBusyConnCounter", metricId);
    this.connEstablishTimer =
        PercentileTimer.get(
            spectatorRegistry,
            spectatorRegistry.createId(METRIC_PREFIX + "_createTiming", "id", metricId));
    this.connsInPool =
        SpectatorUtils.newGauge(METRIC_PREFIX + "_inPool", metricId, new AtomicInteger());
    this.connsInUse =
  ...
  public DefaultClientChannelManager(
      ...
      Resolver<? extends DiscoveryResult> resolver,
    ...
    this.closeAbovePoolHighWaterMarkCounter =
        SpectatorUtils.newCounter(METRIC_PREFIX + "_closeAbovePoolHighWaterMark", metricId);
    this.closeExpiredConnLifetimeCounter =
        SpectatorUtils.newCounter(METRIC_PREFIX + "__closeExpiredConnLifetime", metricId);
    this.requestConnCounter = SpectatorUtils.newCounter(METRIC_PREFIX + "_request", metricId);
    this.reuseConnCounter = SpectatorUtils.newCounter(METRIC_PREFIX + "_reuse", metricId);
    this.releaseConnCounter = SpectatorUtils.newCounter(METRIC_PREFIX + "_release", metricId);
    this.alreadyClosedCounter =
  ...
  public void init() {
    ...
    // We don't do this within the constructor because some subclass may not be initialized until
    // post-construct.
    this.channelInitializer =
        createChannelInitializer(clientConfig, connPoolConfig, spectatorRegistry);
  ...
  public boolean release(final PooledConnection conn) {
    ...
    // TODO(argha-c): Document what is a reasonable default here, and the class of origins that
    // optimizes for
    final boolean connExpiredLifetime =
        conn.getUsageCount() > connPoolConfig.getMaxRequestsPerConnection();
    if (conn.isShouldClose() || connExpiredLifetime) {
      // Close and discard the connection, as it has been flagged (possibly due to receiving a
      // non-channel error like a 503).
  ...
  public static void removeHandlerFromPipeline(
      final String handlerName, final ChannelPipeline pipeline) {
  ...
  protected PooledConnectionFactory createPooledConnectionFactory(
    ...
    return ch ->
        new PooledConnection(
            ch, chosenServer, clientChannelMgr, closeConnCounter, closeWrtBusyConnCounter);
  }
  ...
  static SocketAddress pickAddressInternal(ResolverResult chosenServer, OriginName originName) {
    ...
    try {
      ...
      Counter unresolvedDiscoveryHost =
