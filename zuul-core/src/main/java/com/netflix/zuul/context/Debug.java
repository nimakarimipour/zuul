 ...
 *
 * @author Mikey Cohen Date: 1/25/12 Time: 2:26 PM
 */
public class Debug {
  ...
  public static void addRequestDebugForMessage(
      SessionContext ctx, ZuulMessage message, String prefix) {
   ...
   * @param copy
   */
  public static void compareContextState(
      String filterName, SessionContext context, SessionContext copy) {
    // TODO - only comparing Attributes. Need to compare the messages too.

    // Ensure that the routingDebug property already exists, otherwise we'll have a
    // ConcurrentModificationException below
      ...
      if ((!key.equals("routingDebug") && !key.equals("requestDebug"))) {
        ...
        if (!(newValue instanceof ReferenceCounted) && !(oldValue instanceof ReferenceCounted)) {
          if (oldValue == null && newValue != null) {
            addRoutingDebug(
                context, "{" + filterName + "} added " + key + "=" + newValue.toString());
          } else if (oldValue != null && newValue != null) {
            if (!(oldValue.equals(newValue))) {
              addRoutingDebug(
                  context, "{" + filterName + "} changed " + key + "=" + newValue.toString());
            }
          }
        }
      }
  ...
  public static Observable<Boolean> writeDebugRequest(
    ...
    if (obs == null) obs = Observable.just(Boolean.FALSE);
  ...
  public static Observable<Boolean> writeDebugResponse(
    ...
    if (Debug.debugRequest(context)) {
      ...
      Debug.addRequestDebug(
          context, String.format("%s:: %s STATUS: %s", prefix, arrow, response.getStatus()));
  ...
  public static Observable<Boolean> writeDebugMessage(
    ...
    for (Header header : msg.getHeaders().entries()) {
      Debug.addRequestDebug(
          context,
          String.format("%s:: %s HDR: %s:%s", prefix, arrow, header.getKey(), header.getValue()));
    }
