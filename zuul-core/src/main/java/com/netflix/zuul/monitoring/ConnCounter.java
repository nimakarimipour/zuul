...
public final class ConnCounter {
   ...
   *
   * <p>This can be removed after https://github.com/Netflix/spectator/issues/862 is fixed.
   */
  ...
  public void increment(String event, Attrs extraDimensions) {
    ...
    if (counts.containsKey(event)) {
      // TODO(carl-mastrangelo): make this throw IllegalStateException after verifying this doesn't
      // happen.
    ...
    Id id =
        registry
            .createId(metricBase.name() + '.' + event)
  ...
  public void decrement(String event) {
    ...
    synchronized (getLock(gauge.id())) {
      // Noop gauges break this assertion in tests, but the type is package private.   Check to make
      // sure
  ...
  // This is here to pick the correct lock stripe.   This avoids multiple threads synchronizing on
  // the
  // same lock in the common case.   This can go away once there is an atomic gauge update
  // implemented
  // in spectator.
