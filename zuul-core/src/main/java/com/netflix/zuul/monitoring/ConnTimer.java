...
public final class ConnTimer {
  ...
  private static final AttributeKey<ConnTimer> CONN_TIMER =
      AttributeKey.newInstance("zuul.conntimer");
  ...
  public void record(Long now, String event, Attrs extraDimensions) {
    ...
    timings.forEach(
        (from, stamp) -> {
          ...
          if (durationNanos == 0) {
            // This may happen if an event is double listed, or if the timer is not accurate enough
            // to record
            // it.
            return;
          }
          registry
              .timer(buildId(metricBase, from, event, dimTags))
  ...
  private Id buildId(Id base, String from, String to, Map<String, String> tags) {
    return registry
        .createId(metricBase.name() + '.' + from + '-' + to)
        .withTags(base.tags())
        .withTags(tags);
  }
}
