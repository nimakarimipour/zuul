...
public class CurrentPassport {
  ...
  public static final AttributeKey<CurrentPassport> CHANNEL_ATTR =
      AttributeKey.newInstance("_current_passport");
  private static final Ticker SYSTEM_TICKER = Ticker.systemTicker();
  private static final Set<PassportState> CONTENT_STATES =
      Sets.newHashSet(
  ...
  private Unlocker lock() {
    ...
    if ((historyLock.isLocked() && !historyLock.isHeldByCurrentThread())
        || !(locked = historyLock.tryLock())) {
      Thread owner = historyLock.getOwner();
      String ownerStack =
          String.valueOf(owner != null ? Arrays.asList(owner.getStackTrace()) : historyLock);
      logger.warn(
  ...
  /**
   * NOTE: This is NOT nanos since epoch. It's just since an arbitrary point in time. So only use
   * relatively.
   */
  ...
  public StartAndEnd findFirstStartAndLastEndStates(
  ...
  public StartAndEnd findLastStartAndFirstEndStates(
      PassportState startState, PassportState endState) {
    StartAndEnd sae = new StartAndEnd();
    try (Unlocker ignored = lock()) {
      for (PassportItem item : history) {
  ...
  public boolean wasProxyAttempt() {
    // If an attempt was made to send outbound request headers on this session, then assume it was
    // an
    // attempt to proxy.
...
class CountingCurrentPassport extends CurrentPassport {
  ...
  private static final Counter IN_REQ_LAST_CONTENT_RECEIVED_CNT =
      createCounter("in_req_last_cont_rec");

  private static final Counter IN_RESP_HEADERS_RECEIVED_CNT = createCounter("in_resp_hdrs_rec");
  private static final Counter IN_RESP_LAST_CONTENT_RECEIVED_CNT =
      createCounter("in_resp_last_cont_rec");

  private static final Counter OUT_REQ_HEADERS_SENT_CNT = createCounter("out_req_hdrs_sent");
  private static final Counter OUT_REQ_LAST_CONTENT_SENT_CNT =
      createCounter("out_req_last_cont_sent");

  private static final Counter OUT_RESP_HEADERS_SENT_CNT = createCounter("out_resp_hdrs_sent");
  private static final Counter OUT_RESP_LAST_CONTENT_SENT_CNT =
      createCounter("out_resp_last_cont_sent");
