...
class HttpRequestMessageImplTest {
  ...
  void testOriginalRequestInfo() {
    ...
    assertEquals(
        request.getHeaders().getFirst("Host"), originalRequest.getHeaders().getFirst("Host"));
  ...
  void testGetOriginalHost_handlesNonRFC2396Hostnames() {
    ...
    request =
        new HttpRequestMessageImpl(
            ...
            "localhost");
    assertEquals(
        "my_underscore_endpoint^including~more-chars.netflix.com", request.getOriginalHost());
  ...
  void getOriginalHost_failsOnUnbracketedIpv6Address() {
    ...
    assertThrows(
        URISyntaxException.class, () -> HttpRequestMessageImpl.getOriginalHost(headers, "server"));
  }
  ...
  void testCleanCookieHeaders() {
    ...
    assertEquals(
        " BlahId=12345; something=67890;",
        HttpRequestMessageImpl.cleanCookieHeader(
            " Secure, BlahId=12345; Secure, something=67890;"));
    assertEquals("", HttpRequestMessageImpl.cleanCookieHeader(""));
  }

  @Test
  void shouldPreferClientDestPortWhenInitialized() {
    HttpRequestMessageImpl message =
        new HttpRequestMessageImpl(
            new SessionContext(),
            "HTTP/1.1",
            "POST",
