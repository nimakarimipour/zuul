...
class ClientRequestReceiverTest {
  ...
  void proxyProtocol_portSetInSessionContextAndInHttpRequestMessageImpl() {
    ...
    InetSocketAddress hapmDestinationAddress =
        new InetSocketAddress(InetAddresses.forString("2.2.2.2"), 444);
    channel
    ...
    HttpRequestMessageImpl result;
    {
      channel.writeInbound(
          new DefaultFullHttpRequest(
              HttpVersion.HTTP_1_1, HttpMethod.POST, "/post", Unpooled.buffer()));
  ...
  void parseUriFromNetty_unknown() {
    ...
    HttpRequestMessageImpl result;
    {
      channel.writeInbound(
          new DefaultFullHttpRequest(
              HttpVersion.HTTP_1_1, HttpMethod.POST, "asdf", Unpooled.buffer()));
      result = channel.readInbound();
      result.disposeBufferedBody();
    }
  ...
  void largeResponse_atLimit() {
    ...
    // Figure out the max size, since it isn't public.
    {
      ...
      channel.writeInbound(
          new DefaultFullHttpRequest(HttpVersion.HTTP_1_1, HttpMethod.POST, "/post", buf));
  ...
  void maxHeaderSizeExceeded_setBadRequestStatus() {
    ...
    channel
        .pipeline()
        .addLast(new HttpServerCodec(maxInitialLineLength, maxHeaderSize, maxChunkSize, false));
  ...
  void multipleHostHeaders_setBadRequestStatus() {
    ...
    HttpRequest httpRequest =
        new DefaultFullHttpRequest(HttpVersion.HTTP_1_1, HttpMethod.POST, "/post");
