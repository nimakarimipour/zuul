...
class ProxyEndpointTest {
  ...
  void setup() {
    ...
    request
        ...
        .set(
            ...
            new BasicNettyOriginManager(Spectator.globalRegistry()));
    request.getContext().setRouteVIP("some-vip");
    request.getContext().put(CommonContextKeys.PASSPORT, CurrentPassport.create());
    proxyEndpoint =
        new ProxyEndpoint(request, chc, null, MethodBinding.NO_OP_BINDING, attemptFactory);
  }
  ...
  void testRetryWillResetBodyReader() {
    ...
    // move the body readerIndex to the end to mimic nettys behavior after writing to the origin
    // channel
