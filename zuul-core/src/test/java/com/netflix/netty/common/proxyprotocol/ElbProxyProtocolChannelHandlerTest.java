...
class ElbProxyProtocolChannelHandlerTest {
  ...
  void noProxy() {
    ...
    channel
        .pipeline()
        .addLast(
            ElbProxyProtocolChannelHandler.NAME,
            new ElbProxyProtocolChannelHandler(registry, false));
    ByteBuf buf =
  ...
  void extraDataForwarded() {
    ...
    channel
        ...
        .addLast(
            ElbProxyProtocolChannelHandler.NAME,
            new ElbProxyProtocolChannelHandler(registry, true));
    ByteBuf buf =
        Unpooled.wrappedBuffer(
            "PROXY TCP4 192.168.0.1 124.123.111.111 10008 443\r\nPOTATO"
                .getBytes(StandardCharsets.US_ASCII));
  ...
  void negotiateProxy_ppv1_ipv4() {
    ...
    // TODO(carl-mastrangelo): this check is in place, but it should be removed.  The message is not
    // properly GC'd
    // in later versions of netty.
    assertNotNull(channel.attr(HAProxyMessageChannelHandler.ATTR_HAPROXY_MESSAGE).get());
    assertEquals(
        "124.123.111.111", channel.attr(SourceAddressChannelHandler.ATTR_LOCAL_ADDRESS).get());
    assertEquals(
    ...
    assertEquals(
        "192.168.0.1", channel.attr(SourceAddressChannelHandler.ATTR_SOURCE_ADDRESS).get());
    assertEquals(
